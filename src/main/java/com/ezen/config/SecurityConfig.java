...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	 ...
	 // HttpSecurity 객체 - 이 객체를 이용하여 애플리케이션 자원에 대한 인증과 권한관리를 제어함.
	 
	protected void configure(HttpSecurity security) throws Exception {
		security.authorizeRequests().antMatchers("/").permitAll();  // "/" - 모든 사용자가 접근 가능함.
		security.authorizeRequests().antMatchers("/member/**").authenticated();   // 로그인 성공한 인증사용자만 접근 가능함.
		security.authorizeRequests().antMatchers("/manager/**").hasRole("MANAGER");  // 'MANAGER'권한을 가진 사용자가 접근 가능함.
...
/*
	...
	public void authenticate(AuthenticationManagerBuilder auth) throws Exception {
		...
		String query1 = "SELECT id username, concat('{noop}', password) password, true enabled "
						+ "FROM member WHERE id=?";  // 사용자 정보 조회
		String query2 = "SELECT id, role FROM member WHERE id=?";  // 권한 조회
		
		auth.jdbcAuthentication().dataSource(dataSource)
			.usersByUsernameQuery(query1)   // 인증처리
